...
//     setChatLoading(true);
//     try {
//       const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/ai/chat/${slug}`, {
...
const SafeHtml = ({ html, textColor }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      // Isme humne max-width: 100% aur overflow-x: auto laga diya hai taaki bahar na nikle
      shadow.innerHTML = `
        <style>
          :host { 
            display: block; 
          ...
          /* Image/Video card se bahar na nikle */
          img, video, iframe { max-width: 100%; height: auto; border-radius: 8px; }
          
          /* Agar table ya badi div ho toh horizontal scroll aa jaye */
          .custom-wrapper {
            width: 100%;
            overflow-x: auto;
            -webkit-overflow-scrolling: touch;
          }
          /* Scrollbar ko thoda patla aur stylish banaya */
    ...
    case 'custom':
      return customSections.length > 0 ? customSections.map(section => (
        <div key={section._id} className="px-4 py-5 w-full overflow-hidden" style={{ borderTop: `1px solid ${s.border}` }}>
          <SectionTitle color={s.designationColor}>{section.title}</SectionTitle>
          {/* dangerouslySetInnerHTML hata kar SafeHtml laga diya */}

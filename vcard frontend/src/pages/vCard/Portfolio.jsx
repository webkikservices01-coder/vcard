...
//       else { await axios.post(API, fd, { headers: { 'x-auth-token': token(), 'Content-Type': 'multipart/form-data' } }); toast.success('Created'); }
...
//             <button onClick={openCreate} className="mt-4 bg-black text-white text-sm px-4 py-2 rounded-lg hover:bg-gray-800">Add Item</button>
//           </div>
//         ) : (
...
//                     {item.url ? <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1 text-xs text-gray-600 hover:text-black"><span className="truncate max-w-[120px]">{item.url}</span><ExternalLink className="w-3 h-3 shrink-0" /></a> : <span className="text-gray-400 text-sm">—</span>}
//                   </td>
//                   <td className="px-5 py-4">
...
//                   <input value={form[key]} onChange={e => setForm({...form, [key]: e.target.value})} className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black outline-none" placeholder={placeholder} />
//                 </div>
//               ))}
//               <div>
//                 <label className="block text-sm font-medium text-gray-700 mb-1.5">Description</label>
//                 <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={3} className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black outline-none resize-none" placeholder="Describe this project..." />
//               </div>
//               <div>
...
const Portfolio = () => {
  ...
  // STEP 3: Preview link ke liye user ka slug fetch karna
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) { 
        ...
        // toast.success('Updated'); // Alert hata diya gaya hai
      } else { 
        await axios.post(API, fd, { headers: { 'x-auth-token': token(), 'Content-Type': 'multipart/form-data' } }); 
        // toast.success('Created'); // Alert hata diya gaya hai
      }
      
      setModalOpen(false); 
      fetch();

      // STEP 4: Success par Action Popup trigger karein
      setShowPopup(true);

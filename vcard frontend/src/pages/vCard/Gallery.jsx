...
const Gallery = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // toast.success('Added to gallery'); // Alert hide kiya kyunki ab popup aayega
      setModalOpen(false); // Modal band karein
      setForm({ type: 'image', url: '', image: null }); 
      setPreview('');
      fetchGallery(); // Naya data fetch karein
  ...
  const handleNext = () => {
    setShowPopup(false);
    // Yahan next tab ka route set karein (Jaise ki Testimonials ya QR Code)

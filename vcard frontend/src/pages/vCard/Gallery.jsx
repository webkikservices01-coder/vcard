      ...
      <div className="space-y-5 relative">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => { setModalOpen(true); setForm({ type: 'image', url: '', image: null }); setPreview(''); }}
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <div className={`inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${item.type === 'video' ? 'bg-gray-100 text-gray-700' : 'bg-pink-600 text-white'}`}>
            ...
            >
              ...
              >
                ...
                <div className="p-6 space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {['image', 'video'].map(t => (
                        <motion.button key={t} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => setForm({...form, type: t})}
                          className={`py-2.5 rounded-lg border text-sm font-medium transition-colors capitalize ${form.type === t ? 'bg-pink-600 text-white border-pink-600' : 'border-gray-300 text-gray-600 hover:border-gray-400'}`}>
                          {t === 'image' ? '🖼️' : '🎬'} {t}
                        </motion.button>
                      ))}
                    </div>
                  ...
                  {form.type === 'image' ? (
                    ...
                    <div>
                      ...
                      <input value={form.url} onChange={e => setForm({...form, url: e.target.value})} className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-pink-400 outline-none" placeholder="https://youtube.com/watch?v=..." />
                    </div>
                  )}
                </div>
                <div className="flex justify-end space-x-3 p-6 pt-0">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-pink-600 text-white text-sm font-medium rounded-lg hover:bg-pink-700 disabled:opacity-60">{saving ? 'Adding...' : 'Add'}</motion.button>
                </div>
              </motion.div>
            </motion.div>
          )}

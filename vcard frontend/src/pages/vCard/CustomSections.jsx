...
// 🔥 JADUU YAHAN HAI: Shadow DOM component for CSS Isolation 🔥
// Yeh component user ke HTML/CSS ko baaki website se lock/isolate kar deta hai
const SafeHtml = ({ html, textColor }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      // Agar shadow root nahi hai toh create karo
      if (!shadow) shadow = containerRef.current.attachShadow({ mode: 'open' });
      
      // Inherit font-family and set base text color, uske baad user ka HTML daalo
      shadow.innerHTML = `
        <style>
          :host { 
            display: block; 
...
const CustomSections = () => {
  ...
  // Modal ki jagah Inline form handle karne ke liye state
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      if (res.data?.username) {
        ...
        // Live preview ke liye public data fetch kar rahe hain
          ...
          {formOpen ? (
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              ...
              <div className="p-6 space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Section Title *</label>
                  <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black outline-none transition-all" placeholder="e.g., About My Work" />
                </div>
                <div>
                  ...
                  <textarea value={form.content} onChange={e => setForm({...form, content: e.target.value})} rows={12}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-black outline-none resize-none font-mono bg-gray-50"
              ...
              <div className="flex justify-end space-x-3 p-6 pt-0">
                ...
                <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-black text-white text-sm font-medium rounded-lg hover:bg-gray-800 disabled:opacity-60 transition-colors">{saving ? 'Saving...' : editing ? 'Update' : 'Create'}</button>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">

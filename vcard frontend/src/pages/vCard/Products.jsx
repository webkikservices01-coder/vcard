...
const Products = () => {
  ...
  // STEP 3: Preview link ke liye user ka slug(username) fetch karna
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await axios.put(`${API}/${editing}`, fd, { headers: { 'x-auth-token': token(), 'Content-Type': 'multipart/form-data' } });
        // toast.success('Product updated'); // Alert hata diya kyunki ab popup aayega
      } else {
        await axios.post(API, fd, { headers: { 'x-auth-token': token(), 'Content-Type': 'multipart/form-data' } });
        // toast.success('Product created'); // Alert hata diya kyunki ab popup aayega
      }
      
      setModalOpen(false); // 1. Pehle form wala modal band hoga
      fetchProducts();     // 2. Data refresh hoga

      // STEP 4: Form successfully save hone par Action Popup khulega
      setShowPopup(true);
          ...
          <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
              ...
              <div className="flex justify-end space-x-3 p-6 pt-0">
                ...
                {/* Save/Create Button jispe popup trigger hota hai */}
                <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-black text-white text-sm font-medium rounded-lg hover:bg-gray-800 disabled:opacity-60">
                  {saving ? 'Saving...' : editing ? 'Update' : 'Create'}
                </button>
              </div>

...
const ThemeSwatch = ({ theme, selected, onClick, index }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.96 }}
    className={`relative rounded-xl overflow-hidden border-2 text-left transition-shadow ${
      selected ? 'border-pink-600 shadow-lg shadow-pink-100' : 'border-gray-200 hover:border-gray-300'
    }`}
  >
    ...
    <div className="px-2 py-1.5 bg-white">
      <p className="text-[10px] font-semibold text-gray-700 truncate">{theme.text}</p>
    </div>
    <AnimatePresence>
      {selected && (
        <motion.span
          ...
          transition={springy}
          className="absolute top-1.5 right-1.5 w-4.5 h-4.5 w-[18px] h-[18px] rounded-full bg-pink-600 flex items-center justify-center shadow-sm"
        >
...
const Onboarding = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => navigate('/dashboard/plans', { replace: true }), 1100);
    } catch (err) {
          ...
          <AnimatePresence mode="wait">
            {success ? (
              ...
              >
                ...
                <h2 className="text-xl font-bold text-gray-900 mb-1">Card ready!</h2>
                <p className="text-sm text-gray-500">Ab plan choose karte hain...</p>
              </motion.div>
            ) : (
              <motion.div key={`step-${step}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <span className="text-[11px] font-semibold text-gray-400 shrink-0">Step {step}/2</span>
                </div>

                <p className="text-xs font-semibold text-pink-600 uppercase tracking-wider mb-1.5 inline-flex items-center gap-1">
                    ...
                    >
                      ...
                      >
                        <motion.button
                          ...
                          className="relative w-16 h-16 rounded-full bg-pink-50 flex items-center justify-center overflow-hidden shrink-0"
                        >
                          {!photoPreview && (
                            <motion.span
                              className="absolute inset-0 rounded-full border-2 border-dashed border-pink-300"
                      ...
                      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.21 }}>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Card URL</label>
                        <div className="relative">
                        ...
                        <p className="text-[11px] text-gray-400 mt-1">mycardlink.site/c/{form.slug || 'your-name'}</p>
                      </motion.div>
                      ...
                      <motion.button
                        ...
                        type="button"
                        onClick={goToTheme}
                        className="w-full bg-pink-600 hover:bg-pink-700 text-white font-semibold py-3 rounded-lg transition-colors text-sm inline-flex items-center justify-center gap-1.5 mt-2"
                      >
                        <span>Continue</span>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <motion.button
                          ...
                          type="button"
                          onClick={() => setStep(1)}
                          className="shrink-0 w-11 h-11 rounded-lg border border-gray-200 text-gray-500 hover:text-pink-600 hover:border-pink-300 flex items-center justify-center transition-colors"
                        >
                        ...
                        <motion.button
                          ...
                          disabled={saving}
                          className="flex-1 bg-pink-600 hover:bg-pink-700 text-white font-semibold py-3 rounded-lg transition-colors text-sm disabled:opacity-60 inline-flex items-center justify-center gap-1.5"
                        >
                          {saving ? (
